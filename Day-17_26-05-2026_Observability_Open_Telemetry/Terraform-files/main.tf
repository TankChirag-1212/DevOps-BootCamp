...
module "amp" {
  source       = "./observability/amp"
  cluster_name = var.eks_cluster_name
  tags         = var.tags
}

module "observability_iam" {
  source            = "./observability/iam"
  cluster_name      = var.eks_cluster_name
  name              = "${var.business_division}-${var.environment_name}"
  aws_region        = var.aws_region
  account_id        = data.aws_caller_identity.current.account_id
  amp_workspace_arn = module.amp.amp_workspace_arn
  tags              = var.tags
...
module "amg" {
  source       = "./observability/amg"
  cluster_name = var.eks_cluster_name
  amg_role_arn = module.observability_iam.amg_iam_role_arn
  tags         = var.tags

  depends_on = [module.observability_iam]
}
